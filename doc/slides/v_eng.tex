...
\begin{itemize}
	\item Eliminating the packet copy operations
		\begin{itemize}
			\item [$\Rightarrow$] by using shared memory buffers (\emph{memory \textbf{mapping}})\newline
		\end{itemize}
	\item Eliminating the memory allocations
		\begin{itemize}
			\item [$\Rightarrow$] by using \textbf{ring} buffers\newline
		\end{itemize}
	\item The name of our capturing stack: 
		\begin{itemize}
			\item $ring + mapping = \textbf{ringmap}$
		\end{itemize}
\end{itemize}
\end{frame}
